...
There is one useful protection built into the design. Item-level artefacts, a
too-easy distractor, an option that is longer than the others, a subject a model
happens to know less well, apply equally to every condition, because every
condition sees exactly the same 40 items. So they inflate or deflate the
within-condition gap `S_b - S_d`, but they cancel out of the across-condition
comparisons `delta_cue` and the selective drop. Treat those as the load-bearing
numbers and treat `S_b - S_d` as context.
